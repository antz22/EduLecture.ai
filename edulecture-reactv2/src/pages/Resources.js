...
export default ({
  ...
  useEffect(() => {
    client.get("/api/v1/get-lectures/", {
      headers: {
    ...
    <AnimationRevealPage>
      ...
      <Container>
        <ContentWithPaddingXl>
          ...
          <HeadingText><a href="/subjects">Browse By Subject →</a></HeadingText>
          <Posts>
            {lectures.slice(0, visible).map((lecture, index) => (
              <PostContainer key={index} featured={false}>
                <Post className="group" as="a" href={lecture.image_link}>
                  <iframe title={lecture.video_link} src={lecture.video_link} width="405" height="270" allow="autoplay"></iframe>
                  <Info>
                    <Category>{lecture.subject_name}</Category>
                    <CreationDate onClick={e => handleClick(lecture.user_handle)}>{lecture.user_name}</CreationDate>
