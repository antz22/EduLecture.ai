...
export default ({
  ...
  const handleClick = (subject) => {
    console.log(subject);
    navigate(`/lectures/${subject.name}`);
  }
  ...
  useEffect(() => {
    client.get("/api/v1/get-subjects/", {
      headers: {
    ...
    <AnimationRevealPage>
      ...
      <Container>
        <ContentWithPaddingXl>
          ...
          <Posts>
            {subjects.slice(0, visible).map((subject, index) => (
              <PostContainer key={index} featured={true}>
                <Post className="group" as="a" onClick={e => handleClick(subject)}>
                  <Image imageSrc={subjectImages[subject.name]} />
                  <Info>
                    <Category></Category>
